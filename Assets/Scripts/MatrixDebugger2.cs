...
public class MatrixDebugger2 : EditorWindow
{
	public GameObject target;
	GUIStyle style = new GUIStyle();
	[MenuItem("Window/MatrixDebugger2")]
	static void Init()
	{
        MatrixDebugger2 window = (MatrixDebugger2)EditorWindow.GetWindow(typeof(MatrixDebugger2));
		window.Show();
	}
	...
	private void OnGUI()
	{
		...
		if (target != null)
		{
			...
			DrawMatrix(_Matrix4x4.Translate(target.transform.position));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("Rotation matrix", style);
			DrawMatrix(_Matrix4x4.Rotate(target.transform.rotation));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("Scale matrix", style);
			DrawMatrix(_Matrix4x4.Scale(target.transform.localScale));
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("TRS matrix", style);
			DrawMatrix(_Matrix4x4.TRS(target.transform.position, target.transform.rotation, target.transform.localScale));
		}
	}

	private void DrawMatrix(_Matrix4x4 matrix)
	{
		GUIStyle style = new GUIStyle();
		style.fontSize = 20;
